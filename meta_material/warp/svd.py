...
class SVDFunction(autograd.Function):
    @staticmethod
    def forward(
        ctx: autograd.function.FunctionCtx, F: Tensor
    ...
    def backward(
        ...
        U.grad = (
            wp.zeros_like(U)
            if grad_U is None
            else wp.from_torch(grad_U.contiguous(), dtype=wp.mat33)
        )
        sigma.grad = (
            wp.zeros_like(sigma)
            if grad_sigma is None
            else wp.from_torch(grad_sigma.contiguous(), dtype=wp.vec3)
        )
        Vh.grad = (
            wp.zeros_like(Vh)
            if grad_Vh is None
            else wp.from_torch(grad_Vh.contiguous(), dtype=wp.mat33)
        )
