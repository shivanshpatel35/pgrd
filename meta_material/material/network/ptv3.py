    ...
    def forward(self, input):
        for k, module in self._modules.items():
            ...
            # PyTorch module
                ...
                if isinstance(input, Point):
                    if isinstance(module, nn.BatchNorm1d) and input.feat.shape[0] <= 1:
                        pass
                    else:
...
class PointTransformerV3(PointModule):
    ...
    def forward(self, x, x_mask=None):
        ...
        data_dict = {
            ...
            "coord": coord,
            "grid_size": 0.01,
            "batch": torch.arange(B).repeat_interleave(N).to(feat.device),
        }
        ...
        return point["feat"].reshape(B, N, 64).permute(0, 2, 1)
...
class PTv3Encoder(nn.Module):
    def __init__(
        ...
        super(PTv3Encoder, self).__init__()
        self.global_feat = global_feat
        self.point_transformer = PointTransformerV3(in_channels=channel)

    def forward(self, x, x_mask=None):
        x = self.point_transformer(x, x_mask)
        return x, None, None
