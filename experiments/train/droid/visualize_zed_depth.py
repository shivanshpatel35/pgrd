...
from train.droid.trajectory_utils.misc import load_trajectory
from train.droid.camera_utils.recording_readers.svo_reader import SVOReader


def get_zed_depth(filepath):
    # Get paths
    ...
    for svo_file in svo_files:
        ...
        camera.set_reading_parameters(
            image=True, pointcloud=False, concatenate_images=False
        )
    ...
    traj = load_trajectory(
        ...
        camera_kwargs={
            cam_type: {"depth": True} for cam_type in ["varied_camera", "hand_camera"]
        },
    )
    ...
    images = []
    for serial_num, zed_depth, left_rgb, right_rgb in zip(
        serial_numbers, zed_depths, left_rgbs, right_rgbs
    ):
